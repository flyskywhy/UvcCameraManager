...
module.exports = (router) => {
    ...
    var uploadSuccess = function(fileInfo, userId, fileName) {
        ...
        log.debug(fileInfo.path + ' offset ' + fileInfo.offset + ': end............');
        im.convert([fileInfo.path + '[1]', '-resize', '256x256', fileInfo.thumbnailPath],
            function(err, stdout) {
                if (err) {
    ...
    var checkData = function(fileInfo, userId, filename, successCallback, failedCallback) {
    ...
    function* getUploadInfo(md5, fileName, userId) {
        ...
        if (uploadResource) {
            ...
            url = uploadResource.path.substr(uploadResource.path.lastIndexOf('/upload/'));
            snap = uploadResource.thumbnailPath.substr(uploadResource.thumbnailPath.lastIndexOf('/upload/'));
    ...
    var exception_process = (fileInfo) => {
        fs.stat(fileInfo.path, function(err, stats) {
            if (err) {
                log.error(api.apiPath + api.userUploadPOST.path, 'connect aborted...', err);
                return;
            }
            co(function*() {
                fileInfo.offset = stats.size;
                log.debug(fileInfo.path + ' offset ' + fileInfo.offset + ': aborted............');
                yield fileInfo.save();
                return;
            });
            return;
    ...
    router.post(api.apiPath + api.userUploadPOST.path, function*() {
        ...
        var filename = decodeURIComponent(this.query.filename) || 'unnamed-file.mov';
        ...
        var fileInfo = yield db.transaction(function(t) {
                return co(function*() {
        ...
        if (fileInfo === null) {
            var filePath = newFileName(filename, fileType.split('/')[0] === 'IMG' ? picDir : vidDir);
            var thumbnailName = newFileName(filename.substr(0, filename.lastIndexOf('.')) + '-small.png', thumbnailDir);
        ...
        if ((fileInfo.offset < filesize) && isTransmission) {
                    ...
                    if (part.length) {
                        ...
                        part
                            ...
                            .on('finish', function() {
                                fs.stat(fileInfo.path, function(err, stats) {
                                    if (err) {
                                        log.error(api.apiPath + api.userUploadPOST.path, 'pipe finish...', err);
                                        return;
                                    }
                                    if (filesize === stats.size) {
                                        fileInfo.offset = filesize;
                                        checkData(fileInfo, userId, filename, uploadSuccess, uploadFailed);
                        ...
                        log.debug(fileInfo.path + ' offset ' + fileInfo.offset + ': writing............');
                    }
                }
            } catch (err) {
                log.error('catch :' + err);
            ...
            if (fileInfo.status !== STATUS_FILE_READY) { //极小概率进入
                fileInfo.status = STATUS_FILE_READY;
                yield fileInfo.save();
            }
            if (uploadList.status !== STATUS_FILE_READY) { //删除后恢复
        ...
        var snap = fileInfo.thumbnailPath.substr(fileInfo.thumbnailPath.lastIndexOf('/upload/'));
    ...
    router.delete(api.apiPath + api.userUploadDEL.path, function*() { //TODO 根据materialId删除
        this.checkParams('md5').notEmpty();
        if (this.errors) {
            this.body = this.errors;
            this.status = 400;
