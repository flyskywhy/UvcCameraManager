...
module.exports = (router) => {
    ...
    router.get(api.apiPath + api.devCfgGET.path, function*() {
        ...
        var clientConfig = yield clientConfigTbl.findById(devDbid, {
            attributes: {
                ...
                exclude: [
                    'program_file', 'display_file', 'addr_file', 'hardware_file',
                    'server_url', 'spare_url', 'manage_url', 'file_url', 'is_tcpip',
                    'key', 'coordinate'
                ]
            },
            include: [{
    ...
    router.put(api.apiPath + api.devCfgPUT.path, function*() {
        ...
        this.log.info('devCfgPUT: From ', this.request.host, 'params:', this.params, 'body:', body);
        ...
        var client = yield clientListTbl.findOne({
            ...
            include: [{
                ...
                model: virtualScreenTbl,
                attributes: ['id', 'x0', 'y0', 'width', 'height', 'status', 'tdate',
                    'coefficient', 'b_mode_id', 'match_method', 'cycle'
                ],
                required: true
            }]
        });
    ...
    router.get('/api/v0001/devices/:devDbid/interests/:userInterestDbid', function*() {
        this.checkParams('devDbid').notEmpty().toInt();
    ...
    router.get(api.apiPath + api.devOwnerPunitGet.path, function*() {
        ...
        var vsInfo = yield virtualScreenTbl.findOne({
            ...
            attributes: [
                'b_mode_id', 'cycle', 'match_method', 'rentTimeUnit'
            ]
        });

        var cl = vsInfo.c_client_list;
        if (vsInfo.b_mode_id === 1 || vsInfo.b_mode_id === 3 || vsInfo.b_mode_id === 4) {
            var rs = yield rentSettingTbl.findAll({
                where: {
                    'b_virtual_screen_id': vsDbId,
                    'status': defines.moduleStatus.rentSetting.valid.val,
                },
                attributes: ['circle_type', 'start_time', 'end_time', 'val', 'punits',
                    'rentable_punits', 'max_punits', 'price', 'owner_price', 'type'
                ],
            });
            this.body = {
    ...
    router.put(api.apiPath + api.devOwnerPunitPUT.path, function*() {
        ...
        this.log.info('devOwnerPunitPUT: From ', this.request.host, 'params:', this.params, 'body:', body);
            ...
            if (bh) {
                for (var i = 0; i < bh.length; i++) {
                    ...
                    for (var rs of rentSetting) {
                        //开关机时间重合
                        if ((data.start_time >= data.end_time) ||
                            ((data.start_time < rs.end_time) &&
                                (data.end_time > rs.start_time))) {
                            this.body = [{
                                errCode: 'devOwnerPunitPUT_6',
                                user: 'invalid time!'
                            }];
        ...
        try {
            ...
            if (!vscreen.qr_image) {
                ...
                vscreen.qr_image = yield qrImage.putQrImage(vscreen.id, vscreen.dataValues.id2);
                log.debug('>>> upload qr image of vs:', vscreen.id, ', ', vscreen.qr_image);
                yield vscreen.save({
                    transaction: t
                });
            }
