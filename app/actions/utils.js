...
export const forbidden403 = createAction(types.FORBIDDEN403, () =>
  new Date().getTime(),
);

export const getReducerFromAsyncStorage = createAction(
  types.GET_REDUCER_FROM_ASYNC_STORAGE,
  async () => {
    return storageService
      .multiGet(syncReducer)
      .then((arr) => {
        let ob = {};
        arr.forEach((item) => {
          ob[item[0]] = item[1];
...
export const canOpenURL = createAction(
  types.CAN_OPEN_URL,
  async ({url, resolved, rejected}) => {
    return await requestService.canOpenURL(url);
  },
  ({url, resolved, rejected}) => {
    return {
      resolved,
      rejected,
    };
