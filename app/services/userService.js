...
export function getBindPhoneSend({
    phone
}) {
...
export function getUserChangeDevOwnerVerification(phone) {
    return requestService.put(config.api.userChangeDevOwnerVerificationPUT.path + phone, {});
}

export function getUserForgetPasswdVerification(phone) {
    return requestService.put(config.api.userForgetPasswdVerificationPUT.path + phone, {});
}
...
export function getBindPhoneVerify({
    phone,
    verifycode
}) {
...
export function getUserLoginInfo() {
    return requestService.get(config.api.checkLoginGET.path.replace(/:userDbid/, 0));
}
...
export function changeUserPasswd({
    oldpasswd,
    newpasswd
}) {
...
export function resetUserPasswd({
    phone,
    password,
    verifCode
}) {
...
export function changeUserAddr({
    addr,
    id
}) {
