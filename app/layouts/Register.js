...
class Register extends PureComponent {
    ...
    _onGetVcodePress = () => {
        ...
        actions.checkUserExistence(phone, () => {
            actions.toast('账户已经存在，请不要重复注册');
            this.dismissLoadingModal();
        }, error => {
            if (error.res.status === 400) { //用户不存在
                actions.getVcode(phone, () => {
                    this.dismissLoadingModal();
    ...
    _onRegisterPress = () => {
        ...
        actions.checkUserExistence(phone, () => {
            ...
            if (error.res.status === 400) {
                actions.checkUserExistence(qrCode, () => {  // 分享码
                    actions.toast('TODO: 分享码');
                    this.dismissLoadingModal();
                }, err => {                                 // 产品编码
                    if (err.res.status === 400) {
                        actions.register(phone, password, vCode, () => {
                            this.dismissLoadingModal();
                            actions.toast('注册成功');
                            actions.login(phone, password, () => {
                                actions.saveUserPhone(phone);
                                actions.registerDev(qrCode, 'foobar', '', device => {
                                    actions.configDev(device.id, {
                                        moduleId: qrCode,
                                        name: 'Home',
                                    }, '', () => actions.updateDevices(), () => actions.updateDevices());
                                }, () => {});
                                navigation.goBack(routes[routes.length - 2].key);
                            }, e => {
                                e.res.json && e.res.json().then(result => actions.toast(result.data)).catch(() => {});
                            });
                        }, () => {
                            this.dismissLoadingModal();
                            actions.toast('注册失败');
    ...
    toWebView = () => this.props.navigation.navigate('WebViewLayout', {
        url,
        text: '协议'
    })
