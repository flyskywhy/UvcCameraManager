...
function* resizeImage(userid, bucket, object, t) {
  for (var i = 0; i < cfg.aliyun_mts.input.imageStyle.length; i++) {
    ...
    var filename =
      cfg.aliyun_mts.input.imageStyle[i] +
      '_' +
      object.substr(object.lastIndexOf('/') + 1);
    ...
    calcFileMd5(localFile, function (data, res) {
      co(function* () {
        var imgResize =
          object.substr(0, object.lastIndexOf('/') + 1) +
          res.substr(res.lastIndexOf('/') + 1);
        var uploadRes = yield aliyun.OSSPutObject(
          cfg.aliyun_mts.output.bucket,
          imgResize,
          res,
        );
...
function* materialOssStreamMd5(userid, bucket, object, t) {
  var bytesnum =
    materialOssDesc.md5_type.partial.from +
    materialOssDesc.md5_type.partial.length -
    1;
...
function transcodedVideosName(object) {
  ...
  transFname.push(
    prePath +
      cfg.programTemplate.vgaExt +
      filename +
      cfg.programTemplate.suffixExt,
  );
  transFname.push(
    prePath +
      cfg.programTemplate.qvgaExt +
      filename +
      cfg.programTemplate.suffixExt,
  );
...
function* mediaTranscoded(userid, bucket, object) {
  ...
  try {
    ...
    if (headRes.res.status !== 200 && headRes.res.status !== 304) {
...
var removeMaterial = function* (object, t) {
  ...
  if (mediaType === 'vid') {
    ...
    for (var style of cfg.aliyun_mts.input.imageStyle) {
      fnames.push(
        prePath + style + '_' + object.substr(object.lastIndexOf('/') + 1),
      );
    }
  }
