...
function* _destroyDevice(devId, forced, c_client_list, t) {
    ...
    for (var i = 0; i < rented.length; i++) {
        if ((rented[i].type === 2) || (rented[i].type === 1)) { // 租用
            if (!forced) {
...
var destroyDev = function*(devId, forced) {
    ...
    var res = yield db.transaction(function(t) {
...
function* initScreenMaxPunits() {
    ...
    function* _doInit(t) {
        ...
        for (var rc of rentCounts) {
            ...
            if (rc.dataValues.b_mode_id === 3) {
                max_punits = rc.dataValues.cycle / mode[rc.dataValues.b_mode_id].secondsPerPunit;
            } else {
    ...
    var ret = yield db.transaction(function(t) {
            return co(_doInit(t));
        })
        .catch(function(err) {
