...
var textKeywordFilter = function*(text) {
    ...
    aliUtil.arrayEach.call(this, headers, function(name) {
        canonicalizedQueryString += "&" + name + "=" + aliUtil.popEscape(body[name]);
    });

    var stringToSign = 'POST&%2F&' + aliUtil.popEscape(canonicalizedQueryString.substr(1));

    body.Signature = aliUtil.crypto.hmac(aliyuncfg.secretAccessKey + '&', stringToSign, 'base64', 'sha1');
...
var submitMediaInfoJob = function*(filekey) {
    ...
    body.Signature = aliUtil.crypto.hmac(mtsCfg.cfg.secretAccessKey + '&', stringToSign, 'base64', 'sha1');
...
var queryImageInfo = function*(filekey) {
    ...
    var requrl = reqURL(mtsCfg.input.bucket, mtsCfg.input.location) +
