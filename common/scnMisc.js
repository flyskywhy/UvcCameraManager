...
function orgScnsInfo(screens, flag, pubedIds) {
    ...
    for (var i = 0; i < screens.length; i++) {
        ...
        if (flag === scn_query.publishedScns.val ||
        ...
        screens[i].dataValues.start_datetime = _.min(pluck(slots, 'start_datetime'));
...
function* queryScreens(user, queryflag, condition) {
    ...
    switch (flag) {
        ...
        case scn_query.allScnsAssociatedProg.val:
            ...
            if (flag === scn_query.publishedScns.val ||
                ...
                if (flag === scn_query.rentedScns.val ||
    ...
    if (flag === scn_query.scngroupsAssociatedProg.val ||
        ...
        log.info('User %d query \'%s\',  count:', user.id, queryflag.explain,
            ret.count, pluck(ret.groups, 'id'));
    ...
    var incs = [{
        ...
        include: [{
            model: ClientConfigs,
            attributes: [
                'name', 'location', 'physical_width', 'physical_height',
                'flowrate', 'location_image', 'province', 'city', 'district',
                'offLineMode'
            ]
        }]
    }, {
    ...
    log.info('User %d query \'%s\',  count:', user.id, queryflag.explain, ret.count, pluck(ret.screens, 'id'));
