...
module.exports = (router) => {
  ...
  router.put(
    api.apiPath + api.userVerificationPUT.path + ':mobile',
    function* () {
      var ret;
      ...
      ret = yield dayu.send(
        this.params.mobile,
        defines.smsTemplateCode.userRegisterVerify,
      );
      if (ret === true) {
        ret = {
          err: '获取验证码成功',
  ...
  router.put(
    api.apiPath + api.userForgetPasswdVerificationPUT.path + ':mobile',
    function* () {
      var ret;
      this.checkParams('mobile').notEmpty();
      if (this.errors) {
      ...
      ret = yield dayu.send(
        this.params.mobile,
        defines.smsTemplateCode.changePasswordVerify,
      );
      if (ret === true) {
        ret = {
          err: '获取验证码成功',
  ...
  router.post(api.apiPath + api.userResetPasswdPOST.path, function* () {
    ...
    if (result) {
      ...
      if (user !== null) {
        user.setDataValue(
          'password',
          bCrypt.hashSync(this.request.body.password),
        );
        user.save();
        this.body = {};
        this.status = 200;
