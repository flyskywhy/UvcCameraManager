...
module.exports = (router) => {
  router.delete('/api/v0001/acl_role/:role', function* () {
    ...
    if (roleName === 'all') {
      ...
      for (var item of roles) yield acl.delAclRole(item.name);
    } else {
  ...
  router.get('/api/v0001/alipayLogin', function* () {
    ...
    var redirect;
    yield passport.authenticate('Alipay', function* (error, user, info) {});
    if (!id2) {
      redirect = '/';
    } else {
  ...
  router.get('/auth/wx/callback', function* () {
    var ctx = this;
    yield passport.authenticate('loginByWeixinClient', function* (
      err,
      user,
      info,
    ) {
  ...
  // 验证码登录，获取短信验证码
  router.put(
    api.apiPath + api.loginVerificationPUT.path + ':mobile',
    function* () {
      var ret;
      ...
      ret = yield dayu.send(
        this.params.mobile,
        defines.smsTemplateCode.loginConfirmVerify,
      );
      if (ret === true) {
        ret = {
          err: '获取验证码成功',
